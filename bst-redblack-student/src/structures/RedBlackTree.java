...
public class RedBlackTree<T extends Comparable<T>> extends BinarySearchTree<T> {
	...
	// following,
	// 4.1 If uncle is RED, set color properly based on the instruction.
	// Then check for BST property on grandparent node.
	// 5.2 If uncle is black, and node is inside node,
	...
	// 6.3 If uncle is black, and node is outside node,
	...
	@Override
	public void add(T t) {
		BSTNode<T> node = addTwo(t), parent = getParent(node), grandparent = getGrandparent(node), uncle = getUncle(node);
		
		if(isRoot(node))
			node.setColor(BSTNode.BLACK);
		
		if(parent.getColor() == BSTNode.RED)
		t.co
	...
	private BSTNode<T> getParent(BSTNode<T> node) { // done
		return node.getParent();
	}

	private BSTNode<T> getGrandparent(BSTNode<T> node) { // done
	...
	private BSTNode<T> getUncle(BSTNode<T> node) {
		if (hasUncle(node)) {
			BSTNode<T> grandp = getGrandparent(node);
			return (grandp.getLeft() != null && grandp.getLeft().equals(node.getParent())) ? grandp.getRight()
					: grandp.getLeft();
		}
	...
	private boolean hasParent(BSTNode<T> node) { // done
		return node.getParent() != null;
	}

	private boolean hasGrandparent(BSTNode<T> node) { // done
		boolean out = false;

		if (hasParent(node))
			out = node.getParent().getParent() != null;

		return out;
	}

	private boolean hasUncle(BSTNode<T> node) { // done
		boolean out = false;

		if (hasGrandparent(node)) {
			BSTNode<T> grandp = getGrandparent(node);
			if (grandp.getLeft() != null && grandp.getLeft().equals(node.getParent()))
				out = getGrandparent(node).getRight() != null;
			else
				out = getGrandparent(node).getLeft() != null;
		}
